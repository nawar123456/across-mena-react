...
<div  className='per-container'>  per {bookingObject.container?.toString().toUpperCase().replace('FT', ` FT`)}</div>
      ...
      <div className='item_booking'>
			<span className='btn_value' style={{display:'flex',justifyContent: 'center',color: '#000', fontWeight:'bold',margin:'-33px -10px 4px 15px',columnGap:'1.5px',fontFamily:' var(--font-family-ar-primary)'}}>
          <div fontSize style={{fontFamily:'var(--font-family-ar-primary)',alignSelf:'center',fontSize:'17px'}}>{currency} </div>
		  <span className='calculatedOceanFreight-value'>

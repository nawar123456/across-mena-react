...
const InputMail = forwardRef(
    (
        {isSubmitted,showPassword,handleShowPassword,disapled,stylebox,styleColor,styleTextArea,isTextArea,getInputText,field,value,Icon,  placeholder,title ,handleClick,errorValue,valid},ref
    ) => {
...
<textarea
  ...
  style={{
    height: styleTextArea?.height,
  
  
    padding: (!Icon && '10px'),
    fontFamily:
      i18next.language === 'ar'
        ? 'var(--font-family-ar-primary)'
        : 'var(--font-family-en-primary)',
    fontSize: '16px'
  }}
/>
          ...
          style={{
            borderColor:
            isSubmitted && (!valid || (errorValue && value?.trim() === "")) 
              ? '#f60000' 
              : '',
            padding: ((!Icon || showPassword !== undefined) && '10px'),
            fontSize: '16px'
          }}
          disabled={disapled}
        />

...
const AutoCompleteText = ({loadingCity,titleInput,loadingWriting,handleClickColor,Icon,placeholder,inputf,options,isLoading,errorApi,query,loadingTimerSelected,errorValue,value,field,valueSelect,handleInputText,resetPorts,refSearch,refInput,refDropDown,recentValues,onOptionSelected}) => {
// nawar hasan dsadasda
  ...
  const handleChangeText = (e) => {
    ...
    if (!showRecent) {
    setShowRecent(true); // ✅ ضروري لعرض القائمة مباشرة
  }
...
useEffect(() => {
  ...
  const chooseValue = (item)=>{
    ...
    if (onOptionSelected) {
    onOptionSelected(); // ← أخبر المكوّن بإغلاق القائمة
  }
  ...
  return (
    ...
    <div className='automcomplete-searchbox' >
      ...
      <div   className='automcomplete-searchbox__resultsbox '>
        {/* map */}
        {
        query?.length >=2 && showRecent &&loadingWriting===false ?
        <>
        {
        options?.length > 0   ?
